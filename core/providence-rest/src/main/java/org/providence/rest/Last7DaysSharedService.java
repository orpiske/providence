...
public class Last7DaysSharedService {
    private static final Logger logger = LoggerFactory.getLogger(Last7DaysSharedService.class);
    private final SharedDao sharedDao = new SharedDao();

    public List<Shared> lastSevenDays() {
        try {
            return sharedDao.lastSevenDays();
        }
        catch (Exception e) {
