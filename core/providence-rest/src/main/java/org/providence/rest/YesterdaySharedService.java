...
public class YesterdaySharedService {
    private static final Logger logger = LoggerFactory.getLogger(YesterdaySharedService.class);
    private final SharedDao sharedDao = new SharedDao();

    public List<Shared> yesterday() {
        try {
            return sharedDao.yesterday();
        }
        catch (Exception e) {
